...
RUN echo $TIMESTAMP > /dockerbuildversion.txt \
    ...
    && ls -l /root/.m2/repository/gov/pnnl/proven*/*/* 2>/dev/null\
    && printf "\n\nBuilding proven-cluster\n" \
    && cd /build \
    && git clone https://github.com/pnnl/proven-cluster.git -b 'v1.3.5.1' --single-branch \
